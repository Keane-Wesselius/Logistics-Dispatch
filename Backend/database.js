...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
async function getUserData(client, userEmail) {
	...
	if (userEmail != null) {
		...
		const result = await client.db("main").collection("users").findOne({ email: userEmail });
...
async function createNewUser(client, newUser) {
	...
	const alreadyExists = await client.db("main").collection("users").findOne({ email: newUser.email });
	...
	//This means the user does not exist and we are creating a new user
		...
		const result = await client.db("main").collection("users").insertOne(hashedUser);
        if (result)
        {
...
async function placeOrder(client, orderDetails)
{
    const result = await client.db("main").collection("orders").insertOne(orderDetails);
...
async function getAllPendingOrdersBySupplierId(client, supplierID)
{
    const cursor = await client.db("main").collection("orders").find( {type: "pending", supplierId: ObjectId(supplierID)} );
...
async function getAllPendingOrdersByMerchantId(client, merchantID)
{
    const cursor = await client.db("main").collection("orders").find( {type: "pending", merchantId: ObjectId(merchantID)} );
...
async function confirmOrder(client, orderID)
{
    let result = await client.db("main").collection("orders").findOne( {"_id": ObjectId(orderID)} );
    ...
    if (result)
    {
        ...
        await client.db("main").collection("orders").updateOne({ "_id": ObjectId(orderId) }, { $set: updated });
...
async function getAllConfirmedOrdersBySupplier(client, supplierID)
{
    const cursor = await client.db("main").collection("orders").find( {type: "confirmed", supplierId: ObjectId(supplierID)} );
...
async function getAllConfirmedOrdersByMerchant(client, merchantID)
{
    const cursor = await client.db("main").collection("orders").find( {type: "confirmed", merchantId: ObjectId(merchantID)} );
...
async function getAllConfirmedOrdersForDriver(client)
{
    const cursor = await client.db("main").collection("orders").find( {type: "confirmed"} );
...
async function acceptOrder(client, orderID, driverID)
{
    let result = await client.db("main").collection("orders").findOne( {"_id": ObjectId(orderID)} );
...
async function getAllAcceptedOrdersBySupplier(client, supplierID)
{
    const cursor = await client.db("main").collection("orders").find( {type: "accepted", supplierId: ObjectId(supplierID)} );
...
async function getAllAcceptedOrdersByMerchant(client, merchantID)
{
    const cursor = await client.db("main").collection("orders").find( {type: "accepted", merchantId: ObjectId(merchantID)} );
...
async function getAllAcceptedOrdersByDriver(client, driverID)
{
    const cursor = await client.db("main").collection("orders").find( {type: "accepted", driverId: ObjectId(driverID)} );
...
async function completeOrder(client, orderID)
{
    ...
    if (result)
    {
        ...
        let didUpdate = await client.db("main").collection("orders").updateOne({ "_id": ObjectId(orderId) }, { $set: updated });
...
async function getAllCompletedOrdersByDriver(client, driverID)
{
    const cursor = await client.db("main").collection("orders").find( {type: "completed", driverId: ObjectId(driverID)} );
...
async function getAllCompletedOrdersByMerchant(client, merchantID)
{
    const cursor = await client.db("main").collection("orders").find( {type: "completed", merchantId: ObjectId(merchantID)} );
...
async function getAllCompletedOrdersBySupplier(client, supplierID)
{
    const cursor = await client.db("main").collection("orders").find( {type: "completed", supplierId: ObjectId(supplierID)} );
...
async function updateItemQuantity(client, itemId, quantity) {
	const result = await client.db("main").collection("items").findOne({ "_id": ObjectId(itemId) });
	...
	if (result) {
		...
		if (result.quantity >= quantity) {
			...
			await client.db("Examples").collection("items").updateOne({ "_id": ObjectId(itemId) }, { $set: updated });
		}
...
async function insertNewItem(client, itemInfo) {
	const result = await client.db("main").collection("items").insertOne(itemInfo);
...
async function getItemsBySupplier(client, supplierID)
{
    const cursor = await client.db("main").collection("items").find( {supplierId: ObjectId(supplierID)});
