...
function getActiveConnectionByWebSocketOrToken(
  webSocket: WebSocket,
  token: string | null = null
) {
...
function removeActiveConnectionByWebSocketOrToken(
  webSocket: WebSocket,
  token: string | null = null
) {
...
wss.on("connection", function connection(ws) {
  ...
  ws.on("message", function message(data) {
    ...
    const packetType: string | null = Packets.tryGet(
      packetJSONObject,
      Packets.Constants.TYPE
    );
    if (packetType == null) {
    ...
    let activeConnection = getActiveConnectionByWebSocketOrToken(
      ws,
      Packets.tryGet(packetJSONObject, Packets.Constants.TOKEN)
    );
    if (activeConnection != null) {
      ...
      packetType != Packets.PacketTypes.LOGIN &&
      packetType != Packets.PacketTypes.CREATE_ACCOUNT
    ) {
      console.error(
        "Failed to get active connection from WebSocket for packet which requires active connection. Closing connection."
      );
      ws.close();
      return;
    }
    ...
    const clientUserData =
      activeConnection != null ? activeConnection.userData : null;
    ...
    if (packetType == Packets.PacketTypes.LOGIN) {
      ...
      if (
        ...
        // @ts-ignore
        database
          .getUserData(loginPacket.email)
          .then((userDataJSON: DatabaseUserData) => {
              ...
              bcrypt.compare(
                ...
                function (err, result) {
                  if (result) {
                    ...
                    sendIfNotNull(
                      ws,
                      new Packets.AuthenticationSuccessPacket(
                        loginUserData.accountType,
                        activeConnection.token
                      )
                    );
                  } else {
                    console.log(
                      "Got invalid password: " +
                        loginPacket.email +
                        " " +
                        loginPacket.password
                    );
                    sendIfNotNull(
                      ws,
                      new Packets.AuthenticationFailedPacket(
                        Strings.Strings.INVALID_PASSWORD
                      )
                    );
                  }
                }
        ...
        sendIfNotNull(
          ws,
          new Packets.AuthenticationFailedPacket(Strings.Strings.INVALID_LOGIN)
        );
      }
    } else if (
      ...
      if (clientUserData.isDriver()) {
        database?.getAllOrdersByDriver(clientUserData.id).then((orders) => {
          sendIfNotNull(
            ws,
            new Packets.SetLinkedOrders(JSON.stringify(orders))
          );
        });
        ...
        console.log(
          "Got invalid account type for user data: '" +
      ...
      if (clientUserData.isDriver()) {
        database?.getAllConfirmedOrdersForDriver().then((orders) => {
          sendIfNotNull(
            ws,
            new Packets.SetAllConfirmedOrders(JSON.stringify(orders))
          );
        });
      } else if (clientUserData.isMerchant()) {
        database
          ?.getAllConfirmedOrdersByMerchant(clientUserData.id)
          .then((orders) => {
            sendIfNotNull(
              ws,
              new Packets.SetAllConfirmedOrders(JSON.stringify(orders))
        ...
        database
          ?.getAllConfirmedOrdersBySupplier(clientUserData.id)
          .then((orders) => {
            sendIfNotNull(
              ws,
              new Packets.SetAllConfirmedOrders(JSON.stringify(orders))
      ...
      if (clientUserData.isSupplier()) {
        ...
        console.log(
          "Got invalid account type for action GET_LINKED_ORDERS: " +
            clientUserData.accountType
        );
      }
      ...
      if (clientUserData.isSupplier()) {
        ...
        console.log(
          "Got invalid account type for action ADD_ITEM: " +
            clientUserData.accountType
        );
      }
      ...
      if (clientUserData.isSupplier()) {
        ...
        database
          ?.removeItem(removeItemPacket.itemId)
          .then((removedSuccessfully) => {
        ...
        console.log(
          "Got invalid account type for action REMOVE_ITEM: " +
            clientUserData.accountType
        );
      }
      ...
      if (
        ...
        database.createNewUser(accountPacket).then((createdAccount) => {
          if (createdAccount) {
            sendIfNotNull(
              ws,
              new Packets.AccountCreateSuccessPacket().toString()
            );
          } else {
            // TODO: The error is assumed here, but we should really say exactly what went wrong (existing username, email, etc).
            sendIfNotNull(
              ws,
              new Packets.AccountCreateFailedPacket(
                "Failed to create account, account with username / email already exists."
              ).toString()
            );
          }
        });
      ...
      console.log(
        "Received invalid or unhandled packet from client: " + data.toString()
      );
    }
  });
});
